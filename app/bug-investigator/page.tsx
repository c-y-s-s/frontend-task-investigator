...
const copy = {
  "zh-TW": { title: "貼上錯誤，先找出下一步", intro: "不用先分類 Console 或 Network。把你知道的內容全部貼進來，Agent 會整理證據並提出可驗證的方向。", input: "問題與錯誤資訊", inputHint: "可以混合貼上問題描述、預期行為、Error、Console 和 Network Response。", advanced: "進階設定（選填）", expected: "預期行為補充", console: "Console log 補充", network: "Network Response 補充", run: "開始調查", running: "調查中…", safety: "唯讀分析，不會修改程式碼或建立 PR。", cause: "最可能原因", next: "下一步驗證", signal: "看到什麼代表假設成立", confidence: "信心程度", details: "查看完整證據與其他假設", trace: "查看 Agent 執行軌跡", facts: "已觀察事實", hypotheses: "所有可能原因", actions: "完整驗證步驟", missing: "仍缺少的資訊", files: "受影響檔案", tools: "工具呼叫", stop: "停止條件", checkpoint: "採用後只保存調查方向，不會修改程式碼。", reject: "需要更多證據", approve: "採用這個方向", rejectReason: "請說明還缺少什麼證據", cancel: "取消", confirmReject: "保存原因", stopped: "調查已停止", loading: "正在搜尋程式碼並整理證據", disconnected: "即時連線中斷，請重新執行。" },
  en: { title: "Paste the error. Find the next step.", intro: "No need to classify Console or Network first. Paste what you know and let the Agent organize evidence into a verifiable direction.", input: "Problem and error context", inputHint: "Mix the description, expected behavior, Error, Console, and Network Response here.", advanced: "Advanced details (optional)", expected: "Expected behavior notes", console: "Additional Console log", network: "Additional Network Response", run: "Start investigation", running: "Investigating…", safety: "Read-only analysis. No code edits or pull requests.", cause: "Most likely cause", next: "Next verification", signal: "Signal that supports it", confidence: "Confidence", details: "View complete evidence and other hypotheses", trace: "View Agent trace", facts: "Observed facts", hypotheses: "All hypotheses", actions: "All verification actions", missing: "Missing information", files: "Affected files", tools: "Tool calls", stop: "Stop condition", checkpoint: "Adopting this direction only saves the report. It does not edit code.", reject: "Need more evidence", approve: "Use this direction", rejectReason: "Describe the missing evidence", cancel: "Cancel", confirmReject: "Save reason", stopped: "Investigation stopped", loading: "Searching code and organizing evidence", disconnected: "Live updates disconnected. Run again." },
};
...
export default function BugInvestigatorPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const response = await fetch(`${API_URL}/api/v1/bug-investigations`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title, repository, branch: "main", error_message: incident, console_log: consoleLog, network_context: networkContext, expected_behavior: expectedBehavior, mode, locale }) });
      ...
      source.addEventListener("done", () => { source.close(); setLoading(false); }); source.onerror = () => { source.close(); setLoading(false); setNotice(t.disconnected); };
  ...
  const topCause = report?.hypotheses[0]; const nextAction = report?.verification_actions[0];
  return <main>
    ...
    {item && <section className="bug-simple-results"><div className="bug-result-status"><span>{item.status.replaceAll("_", " ")}</span><span>{item.steps.filter(step => step.status === "completed").length}/{item.steps.length} steps · {item.token_usage} tokens</span></div>{item.error && <div className="error-card"><strong>{t.stopped}</strong><p>{item.error}</p></div>}{!report && !item.error && <div className="analysis-loader"><span /><h3>{t.loading}</h3></div>}{item.rejection_reason && <div className="rejection-note"><strong>{t.reject}</strong><p>{item.rejection_reason}</p></div>}
      {report && topCause && nextAction && <><div className="bug-answer-grid"><article className="bug-primary-answer"><span>{t.cause}</span><h2>{topCause.title}</h2><p>{topCause.explanation}</p></article><article><span>{t.next}</span><h3>{nextAction.action}</h3><p><b>{t.signal}：</b>{nextAction.expected_signal}</p></article><article className="bug-confidence-card"><span>{t.confidence}</span><strong>{report.confidence.level}</strong><p>{report.confidence.reason}</p></article></div>
        <details className="bug-detail-panel"><summary>{t.details}<span>{report.observed_facts.length + report.affected_files.length + report.hypotheses.length}</span></summary><div className="bug-detail-content"><section><div className="report-section-title"><span>{t.facts}</span><b>{report.observed_facts.length}</b></div><ul className="checklist-list">{report.observed_facts.map(fact => <li key={fact}>{fact}</li>)}</ul></section><section><div className="report-section-title"><span>{t.hypotheses}</span><b>{report.hypotheses.length}</b></div><div className="hypothesis-list">{report.hypotheses.map(hypothesis => <article key={hypothesis.rank}><header><b>#{hypothesis.rank}</b><div><h3>{hypothesis.title}</h3><span>{hypothesis.confidence}</span></div></header><p>{hypothesis.explanation}</p><ul>{hypothesis.evidence.map((evidence, index) => <li key={`${evidence.source}-${index}`}><span>{evidence.source}</span>{evidence.observation}{evidence.citation && <> · <a href={evidence.citation.url} target="_blank" rel="noreferrer">{evidence.citation.label} ↗</a></>}</li>)}</ul></article>)}</div></section><section><div className="report-section-title"><span>{t.files}</span><b>{report.affected_files.length}</b></div><div className="bug-file-list">{report.affected_files.map(file => <article key={file.path}><div><code>{file.path}</code><span>{file.risk_level}</span></div><p>{file.reason}</p>{file.citations.map(citation => <a key={citation.url} href={citation.url} target="_blank" rel="noreferrer">{citation.label} ↗</a>)}</article>)}</div></section>{report.missing_information.length > 0 && <section><div className="report-section-title"><span>{t.missing}</span><b>{report.missing_information.length}</b></div><ul className="question-list">{report.missing_information.map(question => <li key={question}>{question}</li>)}</ul></section>}<div className="stop-condition"><strong>{t.stop}</strong><p>{report.stop_condition}</p></div></div></details>
        <details className="bug-detail-panel"><summary>{t.trace}<span>{item.tool_calls.length}</span></summary><div className="bug-trace-grid">{item.steps.map(step => <article key={step.key}><i>{step.status === "completed" ? "✓" : "○"}</i><div><strong>{step.label}</strong><p>{step.summary || "—"}</p></div><small>{step.duration_ms || 0}ms</small></article>)}</div>{item.tool_calls.length > 0 && <div className="bug-inline-tools"><span>{t.tools}</span>{item.tool_calls.map((tool, index) => <code key={`${tool.tool_name}-${index}`}>{tool.tool_name} · {tool.duration_ms}ms</code>)}</div>}</details>
      </>}{report && item.status === "waiting_approval" && <div className="approval-bar bug-simple-approval"><div><span>HUMAN CHECKPOINT</span><strong>{t.checkpoint}</strong></div>{showReject ? <div className="reject-reason"><input value={rejectionReason} placeholder={t.rejectReason} onChange={e => setRejectionReason(e.target.value)} /><button className="reject-button" onClick={() => setShowReject(false)}>{t.cancel}</button><button className="approve-button" onClick={() => decide("reject")}>{t.confirmReject}</button></div> : <><button className="reject-button" onClick={() => setShowReject(true)}>{t.reject}</button><button className="approve-button" onClick={() => decide("approve")}>{t.approve}</button></>}</div>}
    </section>}
  </main>;
}
