...
type Report = { analysis_type: "response" | "openapi"; api_title: string; api_version: string; summary: string; endpoints: Endpoint[]; findings: Finding[]; clarification_questions: string[]; frontend_checklist: string[]; response_fields: ResponseField[]; typescript_draft: string; privacy_warnings: string[]; contract_notes_used: string[]; confidence: { reason: string } };
...
const copy = {
  "zh-TW": { title: "API Response 分析台", description: "貼上回應資料，快速整理型別、空值、分頁與前端風險。", source: "Response JSON", openapiSource: "OpenAPI 文件", purpose: "功能用途", purposeHint: "例如：管理後台申請列表，支援分頁與狀態查看。", knownContract: "已知 API 契約／規則（選填）", knownContractHint: "填入你已知的 Enum、分頁參數、認證、nullable 或錯誤格式；不要貼 Token。", inputResponse: "Response JSON", inputOpenapi: "OpenAPI Document", method: "Method", path: "API Path", fields: "Response 欄位", privacy: "個資風險欄位", contractUsed: "已採用的使用者契約", typeDraft: "TypeScript 型別草稿", run: "開始分析", running: "分析中…", replay: "Replay", live: "Live AI", endpoints: "Endpoints", findings: "分析發現", questions: "待確認問題", checklist: "前端串接清單", approve: "核准報告", reject: "拒絕", back: "Task Investigator", waiting: "等待人工核准", tokens: "Tokens", guideTitle: "這次會得到什麼", guideItems: ["欄位型別與 nullable 判斷", "分頁結構與個資風險", "TypeScript 草稿與待確認問題"], privacyNote: "Live AI 前會先遮罩常見個資值，原始 JSON 分析後不保留。", copyQuestions: "複製問題", copied: "待確認問題已複製", questionHelp: "交給後端或 PM 確認；取得答案後填入『已知 API 契約』再重新分析。" },
  en: { title: "API Response workspace", description: "Paste response data to map types, nullability, pagination, and frontend risks.", source: "Response JSON", openapiSource: "OpenAPI document", purpose: "Feature purpose", purposeHint: "Example: Admin application list with pagination and status display.", knownContract: "Known API contract / rules (optional)", knownContractHint: "Add known enums, pagination params, auth, nullable, or error formats. Never paste tokens.", inputResponse: "Response JSON", inputOpenapi: "OpenAPI Document", method: "Method", path: "API path", fields: "Response fields", privacy: "Personal-data fields", contractUsed: "User-provided contract used", typeDraft: "TypeScript type draft", run: "Run analysis", running: "Analyzing…", replay: "Replay", live: "Live AI", endpoints: "Endpoints", findings: "Findings", questions: "Questions", checklist: "Frontend checklist", approve: "Approve report", reject: "Reject", back: "Task Investigator", waiting: "Waiting for approval", tokens: "Tokens", guideTitle: "What you will get", guideItems: ["Field types and nullable detection", "Pagination and personal-data risks", "TypeScript draft and open questions"], privacyNote: "Common personal values are redacted before Live AI and the original JSON is not retained.", copyQuestions: "Copy questions", copied: "Questions copied", questionHelp: "Send these to the backend engineer or PM. Add their answers to Known API contract, then run again." },
};
...
created_at 保證為包含時區位移的 RFC 3339 字串`);
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const response = await fetch(`${API_URL}/api/v1/api-analyses`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ document, input_type: inputType, purpose: inputType === "response" ? purpose : "", known_contract: inputType === "response" ? knownContract : "", method: inputType === "response" && method ? method : null, path: inputType === "response" && path ? path : null, mode, locale }) });
  ...
  return <main>
    ...
    {(analysis || loading) && <section className="api-results">
      ...
      {report && <div className="api-report">
        ...
        {report.analysis_type === "response" && <><section><div className="report-section-title"><span>{t.fields}</span><b>{report.response_fields.length}</b></div><div className="response-field-list">{report.response_fields.map((field) => <div key={field.path}><code>{field.path}</code><span>{field.inferred_type}{field.nullable ? " · nullable" : ""}</span></div>)}</div></section>{report.contract_notes_used.length > 0 && <section><div className="report-section-title"><span>{t.contractUsed}</span><b>{report.contract_notes_used.length}</b></div><div className="contract-notes">{report.contract_notes_used.map((note) => <pre key={note}>{note}</pre>)}</div></section>}<section className="api-two-columns"><div><div className="report-section-title"><span>{t.typeDraft}</span></div><pre className="type-draft"><code>{report.typescript_draft}</code></pre></div><div><div className="report-section-title"><span>{t.privacy}</span><b>{report.privacy_warnings.length}</b></div><ul className="question-list">{report.privacy_warnings.map((field) => <li key={field}><code>{field}</code></li>)}</ul></div></section></>}
