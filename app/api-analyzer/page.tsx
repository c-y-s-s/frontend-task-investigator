...
const copy = {
  "zh-TW": { title: "分析 API 資料", description: "貼上單支 API Response JSON，或切換為完整 OpenAPI 文件分析。", source: "Response JSON", openapiSource: "OpenAPI 文件", purpose: "功能用途（建議填寫）", purposeHint: "例如：管理後台申請列表，支援分頁與狀態查看。", inputResponse: "Response JSON", inputOpenapi: "OpenAPI Document", method: "Method（選填）", path: "API Path（選填）", fields: "Response 欄位", privacy: "個資風險欄位", typeDraft: "TypeScript 型別草稿", run: "開始分析", running: "分析中…", replay: "Replay", live: "Live AI", endpoints: "Endpoints", findings: "分析發現", questions: "待確認問題", checklist: "前端串接清單", approve: "核准報告", reject: "拒絕", back: "Task Investigator", waiting: "等待人工核准", tokens: "Tokens" },
  en: { title: "Analyze API data", description: "Paste one API response sample, or switch to a complete OpenAPI document.", source: "Response JSON", openapiSource: "OpenAPI document", purpose: "Feature purpose (recommended)", purposeHint: "Example: Admin application list with pagination and status display.", inputResponse: "Response JSON", inputOpenapi: "OpenAPI Document", method: "Method (optional)", path: "API path (optional)", fields: "Response fields", privacy: "Personal-data fields", typeDraft: "TypeScript type draft", run: "Run analysis", running: "Analyzing…", replay: "Replay", live: "Live AI", endpoints: "Endpoints", findings: "Findings", questions: "Questions", checklist: "Frontend checklist", approve: "Approve report", reject: "Reject", back: "Task Investigator", waiting: "Waiting for approval", tokens: "Tokens" },
};
...
export default function ApiAnalyzerPage() {
  ...
  return <main>
    ...
    <section className="workspace api-workspace"><div className="workspace-heading"><div><span>API CONTRACT AGENT</span><h1>{t.title}</h1><p>{t.description}</p></div></div><form className="api-form" onSubmit={submit}>
      ...
      <label><span>{inputType === "response" ? t.source : t.openapiSource}</span><textarea value={document} onChange={(event) => setDocument(event.target.value)} spellCheck={false} /></label><div className="api-form-actions"><div className="mode-picker"><button type="button" className={mode === "replay" ? "selected" : ""} onClick={() => setMode("replay")}>{t.replay}</button><button type="button" className={mode === "live" ? "selected" : ""} onClick={() => setMode("live")}>{t.live}</button></div><button className="run-button" disabled={loading}>{loading ? t.running : t.run}</button></div></form>{notice && <div className="notice">{notice}</div>}</section>
    {(analysis || loading) && <section className="api-results">
      ...
      {report && <div className="api-report">
        ...
        <section className="api-two-columns"><div><div className="report-section-title"><span>{t.questions}</span><b>{report.clarification_questions.length}</b></div><ol className="question-list">{report.clarification_questions.map((item) => <li key={item}>{item}</li>)}</ol></div><div><div className="report-section-title"><span>{t.checklist}</span><b>{report.frontend_checklist.length}</b></div><ul className="question-list">{report.frontend_checklist.map((item) => <li key={item}>{item}</li>)}</ul></div></section>
      </div>}
