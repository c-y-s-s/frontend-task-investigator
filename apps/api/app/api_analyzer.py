...
def analyze_with_openai(summary: dict[str, Any], locale: str, settings: Settings) -> tuple[ApiAnalysisReport, int]:
    ...
    language = """Traditional Chinese used in Taiwan. Write like a frontend engineer in a Taiwanese software team, not a literal translation. Use short, complete sentences and Taiwan terminology such as「程式碼」「欄位」「回應」「請求」「資料」「預設值」「身分驗證」「權限」「錯誤處理」. Avoid Mainland-China wording such as「代码」「字段」「返回」「数据」「默认」「鉴权」「报错」. Keep API, Response, Request, Token, nullable, loading, empty, error, enum, HTTP status codes, paths, and identifiers in their natural technical form. Each clarification question must ask one concrete thing and be ready to send directly to a backend engineer or PM. Do not repeat the same caveat in multiple sections.""" if locale == "zh-TW" else "English"
